...
function integrand_m(T, μ, m, ω, param::Parameters)
    return p -> 4*6*param.Gs*(p^2/2π^2)*m*(1 -  numberF(T, μ, En(p, m) + μ + ω) - numberF(T, μ, En(p, m) - μ - ω))/En(p, m)
end

function integrand_μ(T, μ, m, ω, param::Parameters)
    return p -> 4*6*param.Gv*(p^2/2π^2)*m*(numberF(T, μ, En(p, m) - μ - ω) - numberF(T, μ, En(p, m) + μ + ω))
end

function gapeqns(T, μ, param::Parameters)
    function gap(m, ω)
        return [m - param.m0  - integrate(integrand_m(T, μ, m, ω, param), 0, param.Λ), ω - integrate(integrand_μ(T, μ, m, ω, param), 0, param.Λ) ]
    end
